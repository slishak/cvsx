...
def jallon_2009(only_modify_hr=True):
    p = smith_2007()
    p["cd"].update(hr=jnp.array(54.0))

    if not only_modify_hr:
        p["spt"].update(
            e=jnp.array(convert(3750.0, "mmHg/l")),
            lam=jnp.array(convert(35.0, "1/l")),
        )
        p["vc"].update(e=jnp.array(convert(2, "mmHg/l")))
...
cvs_parameters = {
    "smith": smith_2007(),
    "revie": revie_2012(),
    "jallon": jallon_2009(only_modify_hr=False),
    "jallon_hr_only": jallon_2009(only_modify_hr=True),
}
